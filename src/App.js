...
class App extends Component {
    ...
    $.ajax({
      ...
      success: (searchResults) => {
        ...
        results.forEach((result) => {
          const covidResult = (
            <AllResults key={result.updated} result={result} />
          );
  ...
  submitHandler = (event) => {
    ...
    if (isNaN(this.state.searchTerm)) {
      ...
      $(".country").fadeIn();
      document.getElementById("search-bar__input").placeholder =
        "Enter Country Name or Country ISO";
    } else {
      document.getElementById("search-bar__input").value = "";
      document.getElementById("search-bar__input").placeholder =
    ...
    $.ajax({
      ...
      success: (searchResults) => {
        ...
        results.forEach((country) => {
          const countriesItem = (
            <Countries key={country.country} country={country} />
          );
      ...
      success: (searchResults) => {
        ...
        results.forEach((country) => {
          if (
            ...
            country.alpha2Code.toLowerCase() === searchTerm.toLowerCase() ||
            country.alpha3Code.toLowerCase() === searchTerm.toLowerCase()
          ) {
            document.getElementById(
              "country-population"
            ).innerHTML = this.formatNumber(country.population);
            document.getElementById("country-capital").innerHTML =
              country.capital;
            document.getElementById("country-region").innerHTML =
              country.region;
            document.getElementById("country-timezones").innerHTML =
              country.timezones[0];
            document.getElementById("country-currencies").innerHTML =
              country.currencies[0].name;
            document.getElementById("country-demonym").innerHTML =
              country.demonym;
          }
        });
      },
  ...
  topCountrySearch() {
    ...
    $.ajax({
      ...
      success: (searchResults) => {
        ...
        for (let i = 0; i < 50; i++) {
          const countriesItem = (
            <TopCountries key={cases[i][0]} country={cases[i]} />
          );
      ...
      <Router>
        <div className="App">
            ...
            >
              ...
              <form className="search-bar" onSubmit={this.submitHandler}>
                ...
                <input
                  ...
                  title="Enter Country Name or Country ISO"
                  onChange={this.searchChangeHandler}
                  className="search-bar__input"
                  placeholder="Enter Country Name or Country ISO"
                  type="text"
                  required
                />
                <input
                ...
                <div className="country">
                  <div className="country__information">
                    <div className="country__item">
                      Population:{" "}
                      <span id="country-population" className="country__data">
                        Enter Country
                      </span>{" "}
                    </div>
                    <div className="country__item">
                      Capital:{" "}
                      <span id="country-capital" className="country__data">
                        Enter Country
                      </span>{" "}
                    </div>
                    <div className="country__item">
                      Region:{" "}
                      <span id="country-region" className="country__data">
                        Enter Country
                      </span>{" "}
                    </div>
                    <div className="country__item">
                      Timezone:{" "}
                      <span id="country-timezones" className="country__data">
                        Enter Country
                      </span>{" "}
                    </div>
                    <div className="country__item">
                      Currency:{" "}
                      <span id="country-currencies" className="country__data">
                        Enter Country
                      </span>{" "}
                    </div>
                    <div className="country__item">
                      Demonym:{" "}
                      <span id="country-demonym" className="country__data">
                        Enter Country
                      </span>{" "}
                    </div>
                  </div>
          ...
          <div className="main">
            ...
            <Route
              path="/coronavirusstatus/aboutcorona"
              component={AboutCorona}
            />
            <Route
              path="/coronavirusstatus/prevention"
